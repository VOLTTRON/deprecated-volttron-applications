...
class HASSSwitchAgent(Agent):
    ...
    def __init__(self, config_path, **kwargs):
        ...
        super(HASSSwitchAgent, self).__init__(**kwargs)
    ...
    @PubSub.subscribe('pubsub', record_topic + 'hass/switch/')
    def on_match(self, peer, sender, bus,  topic, headers, message):
        '''
        subscribes to the messages received from HASS Agent about the switch components loaded on HASS API
        '''
    ...
    def on_publish_topic(self):
        '''
            Publishes the information about switch components loaded on HASS API
        '''
            ...
            if(self.data == []):
                ...
                for entry in self.data:
                    ...
                    if(entityId.startswith("switch.")):
                        '''
                            publishes data about switch device
                        '''
    ...
    def TurnOn(self, entityId):
        ...
        urlServices = self.url+'services/switch/turn_on'
        
        try:
            
            jsonMsg = json.dumps({"entity_id" : entityId})
    ...
    def TurnOff(self, entityId):
        '''
            Turns off the switch.entityId device
        '''
        
        if entityId is None:
            return
        
        urlServices = self.url+'services/switch/turn_off'
    ...
    def Toggle(self, entityId):
        '''
            Toggles the switch.entityId device state
        '''
        
        if entityId is None:
            return
        
        urlServices = self.url+'services/switch/toggle'
    ...
    def MysensorsSendIRCode(self, entityId, irCode):
        '''
            Sends an IR code to a mysensors switch device and
            turn it on
        '''
        
        if entityId is None or irCode is None:
            return
        
        urlServices = self.url+'services/switch/mysensors_send_ir_code'
        
        try:
            
            jsonMsg = json.dumps({"entity_id" : entityId, "V_IR_SEND" : irCode})
...
def main(argv=sys.argv):
    ...
    utils.vip_main(HASSSwitchAgent,version=__version__)
